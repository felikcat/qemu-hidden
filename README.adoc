...
----
  ...
  <!-- Helps with how we gave QEMU nice -20 in the libvirt hook. 0-11 is 6 cores 2 threads. -->
  <cputune>
    <vcpusched vcpus="0-11" scheduler="batch"/>
  </cputune>
...
NOTE: CPU pinning is not done anymore as I found out that pinning causes lag issues with the host (non-VM), and has the same performance inside the VM as non-pinning. You will not stutter less by using CPU pinning unless your CachyOS setup is suboptimal, which this guide helps with later on.

* `sudo mkdir -p /etc/libvirt/hooks`

* `git clone https://github.com/felikcat/qemu-hidden`
- sudo python qemu-hidden/generator.py --install-hook --vm-memory 16384`
