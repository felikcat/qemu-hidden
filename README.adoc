...
== About
This guide's goal is to be the easiest to follow, and to bypass some VM detections without slowing down performance. The guide built on CachyOS using systemd-boot or Limine (recommended) and KDE Plasma desktop.

There's currently no custom compiling QEMU in attempts to hide it, it's simply an SMBIOS information and rdtsc exit timing check bypass (that doesn't require kernel patching or have a performance penalty).
...
* `sudo pacman -Syu qemu-desktop virt-manager iptables-nft dnsmasq virglrenderer hwloc dmidecode usbutils swtpm samba`
...
== 3. Optimizing VM performance and hiding the VM from some software
...
----
  ...
  <cputune>
    ...
    <vcpusched vcpus='0-11' scheduler='fifo' priority='99'/>
  </cputune>

  # Replace the inside of <features> with this; if you have issues, take out each line inside hyperv one by one to test (except vendor_id)
    <acpi/>
    <apic/>
    <pmu state="off"/>
    <kvm>
  ...
  # Put inside <qemu:commandline>, this helps with CPU performance. 
...
.If the VM doesn't start with the error "unable to map backing store for guest RAM", run this before starting the VM:
. `sudo bash /etc/libvirt/hooks/qemu YOUR_VM_NAME_HERE started begin -`
