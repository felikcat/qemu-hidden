...
TIP: Bring up the VM called 'win10' that Phantom Hypervisor created, we're going to edit it.

Set or change the following in your VM XML:
----
  # Remove the entire <metadata> and <firmware> block

  # Remove firmware="efi" from <os firmware="efi">, then use the Hypervisor Phantom <loader> and <nvram>
  ...
  # Set either "svm" or "vmx" to "require" if you need nested VMs for further anti-VM detections
